...
def ExportToGNU(file_name):
	...
	for mesh in meshes:
		...
		for i in range(0, 8):
			...
			if zVertMin > boundBoxObjs[i][2]:
				zVertMin = boundBoxObjs[i][2]
			if zVertMax < boundBoxObjs[i][2]:
				zVertMax = boundBoxObjs[i][2]
	...
	file.write("\n")
	file.write("# allows rendering of polygons with hidden line removal\n")
	file.write("set hidden3d back offset 0 trianglepattern 3 undefined 1 altdiagonal bentover\n")
	file.write("\n")
	file.write("# displays borders 0x7F = 0b1111111\n")
	file.write("set border 0x7F linecolor rgb \"#555555\"\n")
	file.write("\n")
	file.write("# displays the x, y and z axis\n")
	file.write("set xzeroaxis linewidth 0.5 linetype 1\n")
	file.write("set yzeroaxis linewidth 0.5 linetype 2\n")
	file.write("set zzeroaxis linewidth 0.5 linetype 3\n")
	file.write("\n")
	file.write("# displays the x, y and z grid\n")
	...
	file.write("set grid ztics linecolor rgb \"#888888\" linewidth 0.2 linetype 9\n")
	file.write("\n")
	
	file.write("# moves the x, y grid to 0\n")
	file.write("set xyplane at 0\n")
	file.write("\n")

	if zVertMin > 0:
		file.write("# moves the x, y grid just below data\n")
		file.write("set ticslevel 0.1\n")
		file.write("\n")

	if zVertMax < 0:
		file.write("# moves the x, y grid just above data\n")
		file.write("set ticslevel -1.1\n")
		file.write("\n")

	file.write("# makes the x, y, and z axis proportional\n")
	file.write("set view equal xyz\n")
	file.write("\n")
	...
	if lockZoomToggle:
		...
		file.write("set zrange ["+str(zVertMin)+":"+str(zVertMax)+"]\n")
		file.write("\n")

	file.write("# moves the key out of the graph\n")
	file.write("set key outside vertical bottom right\n")
	file.write("\n")
	...
	for mesh in meshes:
		...
		if pointLineFaceArray[meshInc] == "p":
			for vInc in range(0, len(mesh.getData().verts)):
				file.write("%f %f %f\n" % (xVert[vInc], yVert[vInc], zVert[vInc]))
				file.write("\n")
	
		if pointLineFaceArray[meshInc] == "l":
			for edges in mesh.getData().edges:
				file.write("%f %f %f\n" % (xVert[edges.v1.index], yVert[edges.v1.index], zVert[edges.v1.index]))
				file.write("%f %f %f\n" % (xVert[edges.v2.index], yVert[edges.v2.index], zVert[edges.v2.index]))
				file.write("\n")
				file.write("\n")
		...
		if pointLineFaceArray[meshInc] == "f":
			for face in mesh.getData().faces:
				if len(face) == 4:
					file.write("%f %f %f\n" % (xVert[face[1].index], yVert[face[1].index], zVert[face[1].index]))
					file.write("%f %f %f\n" % (xVert[face[2].index], yVert[face[2].index], zVert[face[2].index]))
					file.write("\n")
					file.write("%f %f %f\n" % (xVert[face[0].index], yVert[face[0].index], zVert[face[0].index]))
					file.write("%f %f %f\n" % (xVert[face[3].index], yVert[face[3].index], zVert[face[3].index]))
					file.write("\n")
					file.write("\n")
				if len(face) == 3:
					file.write("%f %f %f\n" % (xVert[face[0].index], yVert[face[0].index], zVert[face[0].index]))
...
def gui():
	...
	Draw.Text("'f0.8-#888888-Plane' renders an object with faces, gray edges and a line width of 0.8.")
